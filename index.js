...
// GANTI TOKEN DI BAWAH INI DENGAN TOKEN DARI @BOTFATHER
const token = '8542375348:AAGzEaj-tlfgAJ0PArJPz-x9FQL3EcXyMNw';
const bot = new TelegramBot(token, { polling: true });

// API Forex Factory (Format JSON resmi)
const API_URL = 'https://s3.amazonaws.com/forexfactory.com/calendar/thisWeek.json';
...
if (fs.existsSync(CHATS_FILE)) {
  try {
    ...
    if (Array.isArray(saved)) {
      activeChatIds = new Set(saved);
    }
  } catch (e) {
    console.error('Info: Belum ada chats.json atau gagal muat, menggunakan memory storage.');
  }
}

// Fungsi simpan ID Chat (Aman dari crash server)
function saveChats() {
  try {
    fs.writeFileSync(CHATS_FILE, JSON.stringify(Array.from(activeChatIds)), 'utf8');
  } catch (e) {
    console.log('Storage note: Chat ID disimpan di memory RAM.');
  }
}

// Memory Cache Notifikasi agar tidak terkirim ganda
...
// 2. Command /listnews (Ringkasan Tanggal Berita)
bot.onText(/\/listnews/, async (msg) => {
  ...
  bot.sendMessage(chatId, '⏳ <i>Mengambil data Forex Factory...</i>', { parse_mode: 'HTML' })
  ...
  try {
    ...
    if (!usdNews || usdNews.length === 0) {
      return bot.sendMessage(chatId, '🟢 Tidak ada berita High Impact USD minggu ini.');
    }
    ...
    bot.sendMessage(chatId, '❌ Gagal terhubung ke Forex Factory. Coba lagi nanti.');
  }
});

// 3. Command /news (Detail Jadwal Berita & Forecast)
bot.onText(/\/news/, async (msg) => {
  ...
  bot.sendMessage(chatId, '⏳ <i>Mengambil detail berita XAUUSD...</i>', { parse_mode: 'HTML' })
    .catch(err => console.error(err.message));
  ...
  try {
    ...
    bot.sendMessage(chatId, '❌ Gagal terhubung ke Forex Factory.');
  }
});

// 4. Auto-Check Loop (Pengecekan Otomatis Setiap 1 Menit)
async function checkNewsAutomatically() {
  ...
  try {
    ...
    usdHighNews.forEach((item) => {
      ...
      const newsTime = item.date ? new Date(item.date) : null;

      // A. PENGINGAT 1 JAM SEBELUM RILIS
      if (newsTime && !isNaN(newsTime.getTime())) {
...
// Jalankan auto check setiap 1 menit
setInterval(checkNewsAutomatically, 60 * 1000);

console.log('Bot News XAUUSD siap digunakan!');
